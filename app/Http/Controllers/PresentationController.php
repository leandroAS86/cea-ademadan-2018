...
class PresentationController extends Controller
{
    ...
    public function update(Request $request, $id = null)
    {
        ...
        if (!empty($input))
        {
            ...
            return redirect()
                            ->back()
                            ->with('success', 'Relatório de apresentações de conteúdos específicos sobre mangues atualizado');
        }else 
            return redirect()
                ->back()
   ...
   public function showAttendance($id)
    {   
        $pathToFile = Presentation::where('schedule_id', $id)->value('attendance');
        return response()->file(storage_path().'/app/'.$pathToFile);
    }

    public function showEvidence($id)
    {   
        $pathToFile = Presentation::where('schedule_id', $id)->value('evidence');
        return response()->file(storage_path().'/app/'.$pathToFile);
    }

    public function showReport($id)
    {   
        $pathToFile = Presentation::where('schedule_id', $id)->value('report');
        return response()->file(storage_path().'/app/'.$pathToFile);
    }
}
